...
async def api_extract(request: Request, body: ExtractRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(extract_info, job_id, url, locale)
...
async def api_download(
        ...
        if not user_store.consume_download_free_credit(user["id"]):
            raise HTTPException(
                ...
                detail=t(
                    ...
                    locale,
                    limit=user_store.DOWNLOAD_FREE_LIMIT,
                ),
            )
    ...
    background_tasks.add_task(download_video, job_id, body.format_id, locale)
