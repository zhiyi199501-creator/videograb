...
def test_ai_free_limit_and_consume(db_path):
    user = _make_user("free@example.com")
    uid = user["id"]
    assert user_store.AI_FREE_LIMIT == 3
    assert user_store.can_use_ai(uid) is True
    assert user_store.is_pro_user(uid) is False

    for i in range(3):
        assert user_store.consume_ai_free_credit(uid) is True
        assert user_store.get_ai_free_used(uid) == i + 1

    assert user_store.can_use_ai(uid) is False
    assert user_store.consume_ai_free_credit(uid) is False
    assert user_store.get_ai_free_used(uid) == 3


def test_pro_user_skips_free_credit(db_path):
    ...
    assert user_store.can_use_ai(uid) is True
    assert user_store.consume_ai_free_credit(uid) is True
    assert user_store.get_ai_free_used(uid) == 0
