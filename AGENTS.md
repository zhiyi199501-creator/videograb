...
Stripe 本地测 Webhook：见 `docs/stripe-setup.md`。分开起前后端见 README「分开启动」。上线步骤见 `docs/deploy-online-guide.md`。
...
- 生产容器内勿用 `COOKIES_FROM_BROWSER=chrome`（无浏览器配置目录）；B站/抖音需 `COOKIES_FILE`

## 当前状态 / 下一步

- 已上：下载、AI 总结、登录、Stripe Pro、SEO、海外 Docker + HTTPS 子域名
- 未做：字幕翻译、批量/历史、Team、Job 挂 user_id；B站/抖音 Cookie 运维方案
