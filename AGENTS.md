...
测试与 CI 见 README「测试与 CI」。Stripe：`docs/stripe-setup.md`。上线：`docs/deploy-online-guide.md`。服务器重建：`./scripts/redeploy.sh --pull`。
...
- 进行中：iOS **VideoGrab AI** 精简壳（`ios/`，AI 总结优先、隐藏下载）；需部署前端 `ai-summary` 适配后壳内 UI 才正确；App Store 审核话术按总结工具写
